...
const MediumClap = () => {
    ...
    return (
        <button id={`clap${id}`} className={styles.clap} onClick={handleOnClick}>
            ...
            <ClapCount id={`clapCount${id}`} count={count}/>

            <CountTotal id={`clapCountTotal${id}`} totalCount={countTotal}/>
        </button>
    );
};
...
const ClapCount = ({count, id}) => {
    return <span id={id} className={styles.count}>
        + {count}
    </span>
}

const CountTotal = ({totalCount, id}) => {
    return <span id={id} className={styles.total}>
    {totalCount}
    </span>
}
