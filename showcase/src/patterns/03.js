import React, {useState, useEffect, useId, createContext, useMemo, useContext, useRef} from 'react';
...
const useClapAnimation = (id) => {
    ...
    useEffect(() => {
        ...
        const clap = document.getElementById(`clap${id}`)
...
const MediumClap = ({children, onClap}) => {
    ...
    const animationTimeline = useClapAnimation(id)
    ...
    const memoizedValue = useMemo(() => ({...clapState, id}), [clapState, id])
    ...
    return (
        <Provider value={memoizedValue}>
            <button id={`clap${id}`} className={styles.clap} onClick={handleOnClick}>
                {children}
            </button>
        </Provider>
    );
...
const ClapCount = () => {
    const { count, id } = useContext(MediumClapContext)

    return <span id={`clapCount${id}`} className={styles.count}>
        + {count}
    </span>
}

const CountTotal = () => {
    const { totalCount, id } = useContext(MediumClapContext)
    
    return <span id={`clapCountTotal${id}`} className={styles.total}>
        {totalCount}
    </span>
}
...
const Usage = () => {
    const [{ count, ...clapState }, setClapState] = useState(initialState)

    const handleClapState = (state) => {
        setClapState(state)
    }

    return <div style={{"width": "100%"}}>
        <MediumClap onClap={handleClapState}>
            <MediumClap.Icon />
        ...
        {!!count && <div className={styles.info}>You ve clapped {count} times</div>}
    </div>
}
